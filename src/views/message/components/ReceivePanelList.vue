      ...
      <div class="my-panel_body">
        <div class="main-wrapper">
          <user-title
            ...
            :title="item.sender.name + ' － 老师'"
            :desc="item.created_at | formatTime"
          >
            <div slot="ext" >
              <div v-if="item.reply_status" class="footer-opt-button" style="background: #23d066"><span >已回复</span></div>
              <div
                v-else
                class="footer-opt-button"
                style="background: #FF8C00;"
                @click="handleReply(item.id)"
              >
                <span class="iconfont item-ext-info">我要回复&#xe62d;</span>
              </div>
            </div>
          </user-title>
        ...
        >
          ...
          <div class="reply-ext">
            <span>{{ item.reply_at | formatTime }}</span>
          </div>
        </div>
        <!-- 回复 END -->
      </div>
